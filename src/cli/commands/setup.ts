...
export function readCodexAuthJson(homedir: string = os.homedir()): string | undefined {
  ...
  try {
    ...
    const authMode = typeof parsed.auth_mode === "string" ? parsed.auth_mode.toLowerCase() : undefined;
    const accessToken = parsed.tokens?.access_token;
    const apiKey = parsed.OPENAI_API_KEY;

    if (authMode?.includes("api_key")) {
    ...
    if (authMode?.includes("chatgpt") || authMode?.includes("oauth") || parsed.tokens !== undefined) {
...
export function buildConfigModule({ anthropicApiKey, openaiApiKey, executor }: ApiKeyConfig): string {
  ...
  if (executor !== undefined) {
    lines.push(`  executor: ${JSON.stringify(executor)},`);
  }
...
function buildTsConfigModule(config: OrcaConfig): string {
  return [
...
function buildMergedConfigModule(config: OrcaConfig, useTs: boolean): string {
  if (useTs) {
    return buildTsConfigModule(config);
  }
...
async function saveConfig(
  configPath: string,
  keys: ApiKeyConfig,
  executor: "codex" | "claude" | undefined,
  useTs: boolean
): Promise<void> {
  ...
  const moduleText = useTs
    ? buildTsConfigModule(merged)
    : buildConfigModule({
        ...
        ...(merged.executor !== undefined ? { executor: merged.executor } : {})
      });
  const fullModuleText = buildMergedConfigModule(merged, useTs);
...
function getConfigPath(target: "global" | "project", useTs: boolean): string {
  if (target === "project") {
    return path.resolve(useTs ? "orca.config.ts" : "orca.config.js");
  }

  return path.join(os.homedir(), ".orca", useTs ? "config.ts" : "config.js");
}
...
export async function setupCommandHandler(options: SetupCommandOptions): Promise<void> {
  ...
  try {
    const nestedClaudeCode = isNestedClaudeCodeSession();
    let codexAvailable = Boolean(openaiApiKey) || Boolean(readCodexAuthJson());
    let claudeAvailable = nestedClaudeCode
      ? false
      : (Boolean(anthropicApiKey) || Boolean(readClaudeCodeKeychain()));
    ...
    if (!codexAvailable && !claudeAvailable && rl) {
      ...
      claudeAvailable = nestedClaudeCode
        ? false
        : (Boolean(anthropicApiKey) || Boolean(readClaudeCodeKeychain()));
    }
    ...
    results.push({
      ...
      detail: nestedClaudeCode ? "Claude unavailable (nested Claude Code session)" : (claudeAvailable ? "available" : "not available")
    });
    ...
    const printedPath = target === "global"
      ? (options.ts ? "~/.orca/config.ts" : "~/.orca/config.js")
      : (options.ts ? "./orca.config.ts" : "./orca.config.js");
    if (resolvedExecutor) {
      ...
      if (nestedClaudeCode) {
        console.log("! No usable executors detected in nested Claude Code session. Set OPENAI_API_KEY (Codex) and run `orca setup` again.");
      } else {
